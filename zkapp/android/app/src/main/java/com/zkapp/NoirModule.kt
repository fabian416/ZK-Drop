...
class NoirModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    ...
    @ReactMethod fun prepareSrs(promise: Promise) {
        Thread {
            val localSrsPath = getLocalSrsPath()
            val result = Arguments.createMap()
            result.putBoolean("success", true)
            result.putString("srsSource", if (localSrsPath != null) "local" else "online")
            result.putString("srsPath", localSrsPath ?: "none")
    ...
    Thread {
        ...
        try {
            ...
            promise.reject("SRS_SETUP_FAIL", "Failed to setup SRS for circuit: ${e.message}", e)
        }
        }.start()
    }
